...
class Task {
  ...
  // ─── Format a single task's due_date ──────────────────────
  static formatTaskDates(task) {
    if (!task) return task;
    if (task.due_date) {
      const d = new Date(task.due_date);
      if (!isNaN(d.getTime())) {
        task.due_date = d.toISOString().split('T')[0]; // YYYY-MM-DD
      }
    }
    return task;
  }
  ...
  static formatTasks(tasks) {
    if (Array.isArray(tasks)) {
      return tasks.map(t => this.formatTaskDates(t));
    }
    return this.formatTaskDates(tasks);
  }
  ...
  // ─── Find methods with date formatting ──────────────────

  static async findByUuid(taskId) {
    const [rows] = await pool.query("SELECT * FROM tasks WHERE id = ?", [taskId]);
    if (rows.length === 0) return null;
    return this.formatTaskDates(rows[0]);
  }
